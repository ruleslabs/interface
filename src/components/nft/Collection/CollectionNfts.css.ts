...
export const assetsGrid = style([
  ...
  }),
  {
    ...
    // This means that when the bag and filter panels appear, we no longer get layout thrash and have a consistent
    // animation as the width changes.
    // It uses calc() and subtracts the grid gap to ensure the min size will always fit without wrapping.
    gridTemplateColumns: 'repeat(auto-fill, minmax(calc(100%/4 - 16px), 1fr) )',
    '@media': {
      [`screen and (max-width: ${breakpoints.xxxl}px)`]: {
        gridTemplateColumns: 'repeat(auto-fill, minmax(calc(100%/6 - 16px), 1fr) )',
      },
      [`screen and (max-width: ${breakpoints.xxl}px)`]: {
